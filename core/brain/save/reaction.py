...
class Reaction:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.req_obj = kwargs.get('req_obj')            # get request object
        self.request = self.req_obj.get('request', '')  # request word sequence
        self.req_from = self.req_obj.get('from', '')    # resource(julius..any)
        self.cmd_stack = kwargs.pop('cmd_stack', '')    # get command history
        self.response = ''
    ...
    def run(self):
        ...
        if not self.cmd_stack[-1]:
            request_to_user = 'What do you want me to save exactly ?'
        else:
            request_to_user = 'You wanted me to save "%s" ? It is saved already.' % self.cmd_stack[-1]
