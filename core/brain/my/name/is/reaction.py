...
class Reaction:
    ...
    def run(self):
        ...
        try:
            if self.request.lower().startswith(prefix):
                name = [ self.request.replace(x, '') for x in prefix if x in self.request][0].split()[:2]
                profile.first_name, profile.last_name = name

            else:
                profile.first_name, profile.last_name = self.request.split()
        ...
        response = 'Nice to meet you %s ' % profile.first_name
...
#direct test
#r = Reaction(**{'req_obj':{'request': 'my name is Vasili Pascal', 'from' : '','sender': 'jabberroid@gmail.com' }})
#r.run()

#r = Reaction(**{'req_obj':{'request': 'im is Vasili Pascal', 'from' : '','sender': 'jabberroid@gmail.com' }})
#r.run()

#r = Reaction(**{'req_obj':{'request': 'Vasili Pascal', 'from' : '','sender': 'jabberroid@gmail.com' }})
#r.run()

#r = Reaction(**{'req_obj':{'request': 'Im Vasili Pascal web-developer from webdirect', 'from' : '','sender': 'jabberroid@gmail.com' }})
#r.run()
